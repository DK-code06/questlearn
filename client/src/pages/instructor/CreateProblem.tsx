...
const CreateProblem = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await api.post('/battles/problem', formData);
            alert("Combat Problem Deployed!");
    ...
    return (
        <div className="min-h-screen bg-black text-white p-4 md:p-8 font-sans">
            <div className="max-w-4xl mx-auto bg-gray-900 border border-gray-800 p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-2xl">
                <h1 className="text-xl md:text-3xl font-black text-neon-blue flex items-center gap-2 md:gap-3 mb-6 md:mb-8">
                    <Terminal className="w-6 h-6 md:w-8 md:h-8" /> CREATE BATTLE PROBLEM
                </h1>
                
                <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                    <input required placeholder="Problem Title" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full bg-black border border-gray-700 p-3 md:p-4 rounded-xl text-white outline-none focus:border-neon-blue text-sm md:text-base" />
                    
                    <textarea required rows={5} placeholder="Problem Description / Constraints..." value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full bg-black border border-gray-700 p-3 md:p-4 rounded-xl text-white outline-none focus:border-neon-blue custom-scrollbar text-sm md:text-base" />

                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
                        <div>
                            <label className="text-[10px] md:text-xs text-gray-500 font-bold uppercase mb-1.5 md:mb-2 block">Difficulty</label>
                            <select value={formData.difficulty} onChange={e => setFormData({...formData, difficulty: e.target.value})} className="w-full bg-black border border-gray-700 p-3 md:p-4 rounded-xl text-white outline-none focus:border-neon-blue cursor-pointer text-sm">
                        ...
                        <div>
                            ...
                            <input type="number" min="10" max="50" value={formData.xpReward} onChange={e => setFormData({...formData, xpReward: Number(e.target.value)})} className="w-full bg-black border border-gray-700 p-3 md:p-4 rounded-xl text-white outline-none focus:border-neon-blue text-sm" />
                        </div>
                        <div>
                            <label className="text-[10px] md:text-xs text-gray-500 font-bold uppercase mb-1.5 md:mb-2 block">Medals to Winner</label>
                            <input type="number" min="1" max="5" value={formData.medalReward} onChange={e => setFormData({...formData, medalReward: Number(e.target.value)})} className="w-full bg-black border border-gray-700 p-3 md:p-4 rounded-xl text-white outline-none focus:border-neon-blue text-sm" />
                        </div>
                    </div>
                    ...
                    <div className="pt-5 md:pt-6 border-t border-gray-800">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3 md:mb-4">
                            ...
                            <span className="text-[9px] md:text-[10px] text-gray-600 normal-case bg-black px-2 md:px-3 py-1 rounded-full border border-gray-800 w-fit">Used for scoring</span>
                        </div>
                        ...
                        {formData.testcases.map((tc, idx) => (
                            <div key={idx} className="flex flex-col gap-2 md:gap-3 mb-3 md:mb-4 bg-black/50 p-4 md:p-5 rounded-xl md:rounded-2xl border border-gray-800">
                                <div>
                                    ...
                                    <textarea rows={2} placeholder="e.g. 5\n1 2 3 4 5" value={tc.input} onChange={e => updateTestcase(idx, 'input', e.target.value)} className="w-full bg-black border border-gray-700 p-2 md:p-3 rounded-lg text-xs md:text-sm font-mono outline-none focus:border-neon-blue custom-scrollbar" />
                                </div>
                                
                                <div>
                                    <label className="text-[9px] md:text-[10px] text-gray-500 font-bold uppercase mb-1 block">Expected Output (stdout)</label>
                                    <textarea rows={2} placeholder="Expected Result" value={tc.output} onChange={e => updateTestcase(idx, 'output', e.target.value)} className="w-full bg-black border border-gray-700 p-2 md:p-3 rounded-lg text-xs md:text-sm font-mono outline-none focus:border-neon-blue custom-scrollbar" />
                                </div>
                                ...
                                <div className="flex justify-between items-center mt-1 md:mt-2 pt-2 md:pt-3 border-t border-gray-800">
                                    <label className="flex items-center gap-1.5 md:gap-2 text-[10px] md:text-xs font-bold text-gray-400 cursor-pointer hover:text-white transition-colors">
                                        <input type="checkbox" checked={tc.hidden} onChange={e => updateTestcase(idx, 'hidden', e.target.checked)} className="w-3.5 h-3.5 md:w-4 md:h-4 accent-neon-blue cursor-pointer" /> 
                                        Hidden Testcase
                                    </label>
                        ...
                        <button type="button" onClick={addTestcase} className="bg-gray-900 hover:bg-gray-800 border border-gray-700 text-neon-blue px-3 md:px-4 py-2 md:py-3 rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold uppercase flex items-center justify-center gap-1.5 md:gap-2 w-full transition-colors mt-2">
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-purple-600 hover:bg-purple-500 text-white font-black py-3 md:py-4 rounded-xl flex items-center justify-center gap-2 transition-all mt-6 md:mt-8 active:scale-[0.98] shadow-lg shadow-purple-900/20 disabled:opacity-50 text-sm md:text-base">
                        {loading ? "DEPLOYING SCENARIO..." : <><Save size={16} className="md:w-4.5 md:h-4.5" /> DEPLOY TO ARENA</>}
                    </button>
                </form>
            </div>
        </div>
