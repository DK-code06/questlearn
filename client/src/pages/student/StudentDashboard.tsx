...
const StudentDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans p-4 sm:p-6 md:p-8 selection:bg-neon-blue selection:text-black">
      ...
      {/* HERO CINEMATIC BANNER - Removed max-w to let it stretch */}
          ...
          <div className="w-full max-w-sm bg-white/5 backdrop-blur-2xl border border-white/10 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl relative mx-auto lg:mx-0 mt-4 lg:mt-0">
             <div className="space-y-6 md:space-y-8">
                <div className="flex items-center gap-4 md:gap-6">
                   <div className="w-12 h-12 md:w-14 md:h-14 bg-orange-500/10 rounded-2xl flex items-center justify-center text-orange-500 border border-orange-500/20 shadow-[0_0_20px_rgba(249,115,22,0.1)] shrink-0">
                      <Flame size={24} className="md:w-7 md:h-7" fill="currentColor" />
                   </div>
                   <div>
                ...
                <div className="flex items-center gap-4 md:gap-6">
                   <div className="w-12 h-12 md:w-14 md:h-14 bg-neon-blue/10 rounded-2xl flex items-center justify-center text-neon-blue border border-neon-blue/20 shadow-[0_0_20px_rgba(0,243,255,0.1)] shrink-0">
                ...
                <button 
                  ...
                  className="w-full bg-white text-black font-black py-4 md:py-5 rounded-[1rem] md:rounded-[1.5rem] uppercase tracking-widest text-[10px] hover:bg-neon-blue hover:scale-105 transition-all shadow-xl flex items-center justify-center gap-2 md:gap-3"
                >
                  <ShoppingCart size={16} className="md:w-[18px] md:h-[18px]" /> Visit Store
                </button>
             </div>
          </div>
        </div>
      ...
      {/* Tactical Toolbar - Removed max-w to let it stretch */}
      <div className="w-full flex flex-col xl:flex-row items-center justify-between mb-10 md:mb-16 gap-4 md:gap-6">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 md:gap-4 w-full xl:w-auto">
            <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gray-900 border border-gray-800 hover:border-yellow-500 text-white px-5 md:px-8 py-3 md:py-4 rounded-[1rem] md:rounded-[1.5rem] font-black transition-all group text-[9px] md:text-[10px] uppercase tracking-widest shadow-lg"
            >
            ...
            <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gray-900 border border-gray-800 hover:border-neon-blue text-white px-5 md:px-8 py-3 md:py-4 rounded-[1rem] md:rounded-[1.5rem] font-black transition-all group text-[9px] md:text-[10px] uppercase tracking-widest shadow-lg"
            >
                <Users size={16} className="text-neon-blue group-hover:scale-110 transition-transform" /> Community
            </button>
            <button 
              onClick={() => setIsIssueOpen(true)} 
              className="p-3 md:p-4 bg-gray-900/50 border border-gray-800 rounded-[1rem] md:rounded-[1.5rem] text-gray-600 hover:text-red-500 transition-colors"
            >
              <AlertCircle size={20}/>
            </button>
        </div>
        ...
        {liveCourses.length > 0 && (
          <div className="mb-10 md:mb-16">
              {liveCourses.map(course => (
                  <div key={course._id} className="bg-red-600/5 border-2 border-red-600/30 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] flex flex-col md:flex-row items-center justify-between shadow-[0_0_50px_rgba(220,38,38,0.15)] relative overflow-hidden group gap-6">
                      <div className="absolute inset-0 bg-red-600/5 animate-pulse" />
                      <div className="flex items-center gap-4 md:gap-6 relative z-10 w-full md:w-auto">
                          <div className="bg-red-600 p-4 md:p-5 rounded-2xl md:rounded-3xl shadow-[0_0_30px_rgba(220,38,38,0.4)] shrink-0"><Radio size={28} className="text-white md:w-8 md:h-8" /></div>
                          <div>
                              <h2 className="text-lg md:text-3xl font-black uppercase italic tracking-tighter text-white">LIVE CLASS: {course.title}</h2>
                              <p className="text-red-500 text-[9px] md:text-xs font-black uppercase tracking-[0.2em] md:tracking-[0.4em] mt-1">Instructor is online. Join now.</p>
                          </div>
                      </div>
                      <button 
                          onClick={() => window.open(course.liveSession.meetingLink, '_blank')}
                          className="w-full md:w-auto bg-white text-black px-8 py-4 md:px-10 rounded-xl md:rounded-2xl font-black shadow-2xl hover:bg-red-600 hover:text-white transition-all uppercase tracking-[0.2em] text-[10px] md:text-xs relative z-10 shrink-0"
                      >
                          Join Class
                      </button>
                  </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-10">
              {filteredCourses.map((course) => {
                  ...
                  return (
                      <div key={course._id} className={`bg-gray-900/20 rounded-[2rem] md:rounded-[3rem] overflow-hidden border-2 transition-all duration-500 group flex flex-col ${isLive && isEnrolled ? 'border-red-600 bg-red-600/5' : 'border-gray-800/50 hover:border-neon-blue hover:bg-gray-900/40 shadow-xl hover:shadow-neon-blue/10'}`}>
                          <div className="h-48 md:h-64 bg-black relative overflow-hidden">
                              {course.thumbnail ? (
                                  ...
                                  <div className="w-full h-full flex items-center justify-center text-gray-900 font-black text-[6rem] md:text-[10rem] italic opacity-20">{course.title.charAt(0)}</div>
                              )}
                              <div className="absolute top-4 left-4 md:top-8 md:left-8 bg-black/80 backdrop-blur-md px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[9px] md:text-[10px] font-black text-neon-blue border border-neon-blue/30 uppercase tracking-widest">{course.category}</div>
                              {isEnrolled && <div className="absolute top-4 right-4 md:top-8 md:right-8 bg-green-500 text-black px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-tighter shadow-xl">Enrolled</div>}
                          </div>

                          <div className="p-6 md:p-8 flex flex-col flex-1">
                              <h3 className="font-black text-lg md:text-2xl text-white mb-3 md:mb-4 group-hover:text-neon-blue transition-colors line-clamp-1 uppercase tracking-tight italic">{course.title}</h3>
                              
                              <div className="flex flex-wrap items-center gap-4 md:gap-6 mb-6 md:mb-10">
                                  <div className="flex items-center gap-2 text-yellow-500 font-black text-[10px] md:text-xs">
                                      <Star size={14} className="md:w-4 md:h-4" fill="currentColor" /> {course.averageRating?.toFixed(1) || "0.0"}
                                  </div>
                                  <div className="flex items-center gap-2 text-gray-500 font-black text-[9px] md:text-[10px] uppercase tracking-widest">
                                      <Users size={14} className="md:w-4 md:h-4" /> {course.enrolledStudents?.length || 0} Students
                                  </div>
                              </div>

                              <div className="flex items-center justify-between mt-auto pt-4 md:pt-6 border-t border-gray-800/50">
                                  <div className="flex flex-col">
                                     <span className="text-[9px] md:text-[10px] font-black text-white tracking-widest">{course.sections?.length || 0} MODULES</span>
                                  </div>
                                  <button
                                      onClick={() => navigate(`/student/course/${course._id}`)}
                                      className={`px-6 py-3 md:px-8 md:py-4 rounded-xl md:rounded-2xl font-black text-[9px] md:text-[10px] uppercase tracking-[0.2em] md:tracking-[0.3em] transition-all active:scale-95 shadow-lg md:shadow-2xl ${
